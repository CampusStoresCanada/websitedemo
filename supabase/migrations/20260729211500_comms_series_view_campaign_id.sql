...
select
  series_key,
  series_label,
  count(distinct campaign_send_id) as send_count,
  ...
  max(series_campaign_id::text)::uuid as campaign_id
from (
  select
    c.id as campaign_send_id,
    coalesce(c.template_id::text, 'name:' || c.name) as series_key,
    coalesce(t.name, c.name) as series_label,
    t.campaign_id as series_campaign_id,
...
create or replace view public.comms_campaign_totals as
select
  cc.id as campaign_id,
  count(distinct mc.id) as send_count,
  count(d.id) as total_deliveries,
  count(*) filter (where d.status in ('sent', 'delivered')) as sent_count,
  count(*) filter (where d.status = 'delivered') as delivered_count,
  count(*) filter (where d.open_count > 0) as opened_count,
  count(*) filter (where d.click_count > 0) as clicked_count,
  count(*) filter (where d.status in ('bounced', 'failed')) as failed_count,
  count(*) filter (where d.status = 'complained') as complained_count,
  max(coalesce(mc.completed_at, mc.sent_at, mc.created_at)) as last_sent_at
from comms_campaigns cc
left join message_campaigns mc on mc.campaign_id = cc.id
left join message_deliveries d on d.campaign_id = mc.id
group by cc.id;
