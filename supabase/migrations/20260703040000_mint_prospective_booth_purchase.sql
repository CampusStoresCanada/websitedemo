-- Mints a booth (and whatever it bundles — registrations, table, chairs)
-- for an org created via the pay-first prospect flow. Neither existing mint
-- function fits: mint_v3_for_order() requires a real conference_orders row
-- (this org never went through the cart), and mint_entity_offer_purchase()
-- never sets organization_id on the resulting entity_balances row. This
-- mirrors mint_v3_for_order()'s per-item body exactly (same effective_includes
-- expansion, same booth-keeps-balance-not-seat rule), just parametrized
-- directly instead of reading from conference_order_items.
