-- Allow canceled → active.
--
-- `canceled` was terminal (ALLOWED_TRANSITIONS.canceled = []), so an org that
-- paid for a new coverage period after being canceled had no legal way back:
-- activateMembershipRenewal set the new membership_expires_at and the status
-- stayed `canceled` forever. Roaring Spring paid a 2026-08-31 → 2027-08-31
-- partnership on 2026-08-31 and sat canceled behind it.
--
-- Also clears canceled_at when leaving `canceled`, in both the organizations
-- row and the mirrored memberships row. Without that, a revived org keeps a
-- populated canceled_at and reads as canceled to anything checking the
-- timestamp rather than the status.
--
-- Self-serve renewal is deliberately NOT widened by this: the renewableStatuses
-- gate in lib/actions/renewal.ts is a separate list and still excludes
-- `canceled`, so a deliberate opt-out cannot un-cancel itself by paying. Only
-- an admin/webhook-initiated settlement reaches this transition.
